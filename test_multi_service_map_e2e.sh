...
if ! curl -s http://localhost:8888/status/200 > /dev/null 2>&1; then
    ...
    $DOCKER_COMPOSE -f docker-compose.test.yml up -d mock-http-server 2>/dev/null || true
...
if docker exec opa-mysql mysqladmin ping -h localhost --silent 2>/dev/null; then
    ...
    $DOCKER_COMPOSE -f docker-compose.test.yml up -d mysql-test 2>/dev/null || true
...
test_output_a=$($DOCKER_COMPOSE -f docker-compose.test.yml run --rm \
    ...
    php php -d opa.service=api-gateway \
        ...
        /var/www/html/tests/test_multi_service_map_e2e.php 2>&1)
...
test_output_b=$($DOCKER_COMPOSE -f docker-compose.test.yml run --rm \
    --entrypoint /usr/local/bin/docker-entrypoint-custom.sh \
    -e OPA_ENABLED=1 \
    -e OPA_SOCKET_PATH=opa-agent:9090 \
    -e OPA_SAMPLING_RATE=1.0 \
...
test_output_c=$($DOCKER_COMPOSE -f docker-compose.test.yml run --rm \
    --entrypoint /usr/local/bin/docker-entrypoint-custom.sh \
    -e OPA_ENABLED=1 \
    -e OPA_SOCKET_PATH=opa-agent:9090 \
    -e OPA_SAMPLING_RATE=1.0 \
