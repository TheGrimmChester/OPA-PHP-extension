...
1. **Build the debug container:**
   ```bash
   docker-compose -f docker/compose/docker-compose.debug.yaml build --no-cache
   ```

2. **Start the debug container:**
   ```bash
   docker-compose -f docker/compose/docker-compose.debug.yaml up -d
   ```

3. **Run a test:**
   ```bash
   docker exec -it opa-php-debug php /app/tests/e2e/curl/curl_profiling_test.php
   ```

4. **Analyze the core dump:**
   ```bash
   ./scripts/dev/debug_core.sh
   ```
...
```bash
docker exec -it opa-php-debug /usr/local/bin/debug_interactive.sh /app/tests/e2e/curl/curl_profiling_test.php
```
